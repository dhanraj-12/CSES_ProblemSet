...
public:
    PrimeGenerator() { createBaseSieve(); }
...
ll arraysum(vector<ll> &v) {
    ll ans = 0;
    for (ll i = 0; i < (ll)v.size(); i++) ans += v[i];
    return ans;
}
...
vector<ll> find_all_divisors(ll n) {
    ...
    for (ll i = 1; i <= s; i++) {
        if (n % i == 0) {
            ...
            else { x.push_back(i); x.push_back(n / i); }
        }
    }
    return x;
}
...
ll moduloinverse(ll a,ll m) { return power(a,m-2,m); }
...
void solve() {
    ...
    vi dp(n+1,1e9); // dp[i] --> number of minimum steps to make i to 0;
    dp[0] = 0;
